<template>
  <b-row class="info-pane">
    <b-col sm="8" class="user-header">
      ...
      <h6><strong>Status</strong>: <span class="user-status" v-b-tooltip :title="statusDescription">{{ user.User.UserStatus }}</span></h6>
      <h6><strong>MFA via</strong>: {{ user.MFAPreferences.EnabledOptions ? user.MFAPreferences.EnabledOptions.map(op => op.AttributeName).join(', ') : 'Disabled' }}</h6>
      <h6>
        <strong>Created</strong>: {{ (new Date(user.User.UserCreateDate)).toLocaleString() }}
        /
        <strong>Last modified</strong>: {{ (new Date(user.User.UserLastModifiedDate)).toLocaleString() }}
      </h6>
    </b-col>
    <b-col sm="4">
    ...
    computed: {
      displayName() {
        if (getAttribute(this.user.User.Attributes, "name") && getAttribute(this.user.User.Attributes, "family_name")) {
          return `${getAttribute(this.user.User.Attributes, "name")} ${getAttribute(this.user.User.Attributes, "family_name")}`;
        }
